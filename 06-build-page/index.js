   ...
   for (let i=0;i<data.length;i++){
      ...
      fs.readdir(path.join(assetsFirst,data[i]), (err,file) =>{
         ...
         for(let j=0;j<file.length;j++){
            fs.copyFile(`${path.join(assetsFirst,data[i],file[j])}`, `${path.join(assetsProject, data[i],file[j])}`,callback);
         }         
      });
    }
 });
   ...
   for(let i=0;i<file.length;i++){
     ...
     let regex = new RegExp (name,'gi');
      fs.readFile(path.join(components,file[i]),'utf-8', (err,comp) =>{
         if(err) throw err;
         let indexText = '';
         const index = new fs.createReadStream (indexProject,'utf-8');

         index.on('data', (chunk)=>{
            indexText += chunk.toString().replace(regex,comp);
         });
         index.on('end', () => {
            fs.writeFile(indexProject,indexText, callback);
         });
         // fs.readFile(indexProject, 'utf-8', (err,index) =>{
         //    if(err) throw err;
            
         //   indexText = index.replace(regex, comp);
         //   //index = indexText;
         //  //fs.appendFile(indexProject,indexText,callback);
         //   // console.log(index); 
         // });
